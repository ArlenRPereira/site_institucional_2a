...
>
> **Revisão v1.3 (Balanceamento Tecnologia/ESG no Hero e em Problemas):** incorpora a atualização das telas "Tela Inicial" e "Problemas que resolvemos" do SDD original (`SDD_site_institucional_2ADesenvolvimento.md` v1.2, seção 2.6 "Balanceamento das frentes no site"). Princípio novo: a frente **ESG, Desenvolvimento Institucional e Gestão de Projetos Sociais** não pode mais aparecer só como item secundário — ela precisa ter o **mesmo peso estratégico** que Tecnologia e Produtos Digitais já a partir do Hero, refletindo nos "Problemas que resolvemos" e demais seções. Isso muda o título/subtítulo/frase complementar do Hero (§1.1, §5) e reestrutura "Problemas que resolvemos" de uma lista única de 8 itens (7 de Tecnologia + 1 de ESG) para dois grupos balanceados de 7 itens cada (§5). Revisão já implementada no `produto-web`: `src/data/company.ts` (objeto `hero`) tem o novo título/subtítulo/complemento, e `src/data/problems.ts` foi reestruturado em `problemGroups` (dois grupos de 7 itens); `Problems.tsx` agora renderiza os dois grupos lado a lado (`lg:grid-cols-2`), cada um com seu próprio heading e numeração 01–07 independente.
...
**Balanceamento das frentes (v1.3):** a comunicação da página inicial deve apresentar Tecnologia e Produtos Digitais e ESG/Desenvolvimento Institucional/Gestão de Projetos Sociais com o **mesmo peso estratégico** — a frente de Tecnologia não deve dominar sozinha a mensagem principal. ESG deve aparecer desde o Hero, na seção de problemas resolvidos, nos exemplos de soluções, nos diferenciais e nas chamadas para contato (ver Revisão v1.3 no topo do documento).

**Frases-âncora do posicionamento:**
```
Principal (Hero):        Unimos tecnologia, produtos digitais e projetos sociais para transformar ideias, processos e iniciativas em resultados mensuráveis.
Complementar (Hero):     Da ideia à validação. Do projeto à execução. Da operação ao impacto mensurável.
Institucional ampla:     Tecnologia, inovação, desenvolvimento institucional e impacto social para empresas, startups, instituições e governos.
```
...
- **Dados exibidos:** conteúdo institucional fixo, definido em `src/data/` — não deve ser hardcoded nos componentes de seção (§6)
  ...
  - **Hero** (`#inicio`): título "Unimos tecnologia, produtos digitais e projetos sociais para transformar ideias, processos e iniciativas em resultados mensuráveis." (Tecnologia e ESG no mesmo nível — v1.3, §1.1), subtítulo "A 2A Desenvolvimento e Tecnologia ajuda empresas, startups, prefeituras e instituições a criar soluções digitais, validar MVPs, automatizar operações, estruturar projetos ESG e gerir programas sociais com indicadores, governança e impacto.", frase complementar "Da ideia à validação. Do projeto à execução. Da operação ao impacto mensurável.", botões "Solicitar uma conversa inicial" / "Conhecer soluções", badges das duas frentes
  ...
  - **Problemas que resolvemos** (`#problemas`): dois grupos balanceados de 7 dores reais cada, lado a lado (v1.3, §1.1) (`src/data/problems.ts` → `problemGroups`; renderizado por `Problems.tsx` em `lg:grid-cols-2`, cada grupo com heading próprio e numeração 01–07 independente):
    - **Tecnologia e Produtos Digitais:** ideia de app/plataforma sem direção; MVP não validado antes de investir no produto completo; apresentação a investidores/parceiros; operação em planilha/WhatsApp/processos manuais; automação de tarefas repetitivas e integração de sistemas; sistema antigo, lento, inseguro ou difícil de evoluir; necessidade de solução segura, escalável e performática
    - **ESG, Desenvolvimento Institucional e Projetos Sociais:** estruturar projeto social/ESG com clareza, método e governança; transformar iniciativa social em projeto formal com objetivos, etapas e indicadores; prefeitura/instituição precisa de diagnóstico territorial e plano de ação; acompanhar resultados, evidências, relatórios e prestação de contas; programas sociais com informação descentralizada e pouca visibilidade de impacto; indicadores para demonstrar impacto social/institucional/territorial; unir projeto social, gestão e tecnologia para melhorar acompanhamento e tomada de decisão
...
- As duas frentes de atuação (Tecnologia e Produtos Digitais; ESG, Desenvolvimento Institucional e Gestão de Projetos Sociais) devem ser sempre apresentadas como complementares, nunca como empresas ou marcas separadas, e com **o mesmo peso estratégico a partir do Hero** — Tecnologia não deve dominar sozinha a mensagem principal (v1.3, §1.1).
